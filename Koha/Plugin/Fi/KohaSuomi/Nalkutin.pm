package Koha::Plugin::Fi::KohaSuomi::Nalkutin;
...
sub configure {
    ...
    unless ( $cgi->param('save') ) {
        ...
        $template->param(
            ignore_marc_fields => $self->retrieve_data('ignore_marc_fields'),
            skip_encoding_levels => $self->retrieve_data('skip_encoding_levels'),
        );
        ...
        $self->store_data(
            {
                ignore_marc_fields => $cgi->param('ignore_marc_fields'),
                skip_encoding_levels => $cgi->param('skip_encoding_levels'),
            }
        );
        $self->go_home();
    }
		...
		my $fcode = $tag . $subfield . $type;
			...
			my $efcode =  $eq_tag . $type;
                        ...
                        if (ref($data->{'allow_regex'}{$efcode}{$eq_pos}) eq 'ARRAY') {
                            ...
                            warn "allow_regex equals is not array for '$eq_tag' '$type' '$eq_pos'"
                        }
                    }
                }
            }
...
sub parse_MARC21_format_definition {
    ...
    my %tmpignores = map {($_, 1)} generate_tag_sequence($ignore_marc_fields); #C4::Context->preference('MARC21FormatWarningsIgnoreFields'));
...
sub CheckMARC21FormatErrors {
    ...
    my $skip_enclevels = $self->retrieve_data('skip_encoding_levels') || ''; #"8"; # Record encoding levels (ldr/17) to skip
    if (index($skip_enclevels, substr($record->leader(),17,1)) != -1) {
	return \@errors;
    }

    my $formatdata = parse_MARC21_format_definition($self);
...
sub before_addbiblio_errors {
    my ($self, $args) = @_;

    my @ret;
    my $errors = CheckMARC21FormatErrors($self, $args->{'record'});
